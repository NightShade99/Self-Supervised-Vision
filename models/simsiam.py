...
class OnlineNetwork(nn.Module):
    ...
    def __init__(self, encoder, encoder_dim, projection_dim, bottleneck_dim):
        ...
        self.pred_head = nn.Sequential(
            nn.Linear(projection_dim, bottleneck_dim),
...
class SimSiam:
    ...
    def __init__(self, args):
        ...
        output_root = os.path.join("outputs/simsiam", args["arch"])
        ...
        self.online_network = OnlineNetwork(encoder(**self.config["encoder"]), encoder_dim, self.config["proj_dim"], self.config["bottleneck_dim"]).to(self.device)
        ...
        self.loss_fn = losses.SimSiamLoss()
        self.best_metric = 0
